...
export async function GET(req: Request, { params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const slugArr = slug;
  const blogRoot = path.join(/*turbopackIgnore: true*/ process.cwd(), 'content/blog');
  ...
  // Only one segment for blog, e.g., introducing-bread-modular.md
  const clean = slugArr.join('/');
  const found = await findWithPrefix(blogRoot, clean);
  if (!found) {
    return new Response('Not found', { status: 404 });
  }
  const filePath = path.join(/*turbopackIgnore: true*/ blogRoot, found);
  try {
